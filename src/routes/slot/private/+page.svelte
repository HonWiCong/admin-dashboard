<script lang="ts">
	...
	async function fetchData() {
		const res = await fetch('/api/private/slots');
...
<div class="p-4">
	<h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Private Parking</h1>
</div>
...
<div class="p-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
	{#each data.rows.slice(0, 5) as slot}
		<Card class="h-[150px] sm:h-[200px]" color={getStatusColor(slot.status)}>
